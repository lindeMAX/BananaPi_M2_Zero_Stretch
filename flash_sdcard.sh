...
pt_ok "Done - Geometry created and sd card '$out' formatted"
...
tar -xvpzf rootfs_4.18.6.tar.gz -C ./erootfs --numeric-ow
sync
sudo umount erootfs
rm -fR erootfs
sync
...
tar -xvpzf boot_4.18.6.tar.gz -C ./eboot  --numeric-ow
sync
sudo umount eboot
rm -fR eboot
sync
